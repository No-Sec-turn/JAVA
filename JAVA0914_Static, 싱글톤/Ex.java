...
public class Ex {
	public static void main(String[] args) {
		...
		MiddleClass	mc = sc;
		mc.normalMethod();     //상속받은 일반 메서드
		mc.abstractMethod1();  // MiddleClass 에서 구현한 추상 메서드
		mc.abstractMethod2();  // SubClass에서 구현한 추상 메서드
		
		AbstractClass ac = sc;
		ac.normalMethod();     //상속받은 일반 메서드
		ac.abstractMethod1();  // MiddleClass에서 구현한 추상 메서드
		ac.abstractMethod2();  // SubClass에서 구현한 추상 메서드
	}
}
...
// => MiddleClass 에서 구현되지 않은 abstractMethod2() 메서드에 대한 구현 책임이 발생함! => 반드시 오버라이딩
