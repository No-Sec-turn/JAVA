...
public class Ex {
	public static void main(String[] args) {
		...
		/*
		 * 제네릭(Generic) => 일반화(Generalization) 
		 * - 컴파일 시점에 사용가능한 객체의 타입을 체크하는 것 
		 * - 클래스 또는 인터페이스를 정의할 때 해당 클래스 또는 인터페이스에서 사용하기 
		 *   위한 어떤 타입을 미리 지정하는 것이 아니라 제네릭 타입으로 선언한 후
		 *   실제 객체 사용을 위해 인스턴스를 생성하는 시점에서 사용할 타입을 결정하는 것 
		 * - 자바에서 제공하는 컬렉션 프레임워크 등 클래스 및
		 *   인터페이스에는 제네릭이 적용된 경우가 많으며, 
		 *   이 클래스 등의 인스턴스 생성 시 실제 사용할 데이터 타입을 지정해줘야한다..
		 * 
		 * 
		 */
