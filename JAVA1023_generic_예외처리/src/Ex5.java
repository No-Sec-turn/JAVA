...
//			System.out.println(num / num2); // 예외 발생 예상 코드
		...
		// 2-1. 하나의 catch 블록에 2개 이상의 예외클래스를 모두 지정할 경우
		...
		try {
			...
			System.out.println(str.length());  //여기서 에러 발생해서 catch로 빠진다..
			System.out.println(str);
		...
		// 2. 두 가지 이상의 예외를 하나의 catch 블록에서 처리
		// 2-2. 하나의 catch 블록에 상위 예외클래스를 사용하여 처리
		System.out.println("프로그램 시작!");
...
//			System.out.println("RuntimeException 계열만 처리!");
		...
		//    하위 예외 처리 catch 블록이 실행될 수 없게 된다.
		
		System.out.println("프로그램 종료!");
		
		
		// try문 내의 에러 코드가 없으면 catch문이 실행 되지 않는다!
		try {
			System.out.println("에러 없어요");
		} catch (Exception e) {
