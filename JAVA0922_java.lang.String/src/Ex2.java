...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 *      StringBuilder 또는 StringBuffer 클래스를 활용해야한다!
		 */
		...
		if(str1.equals(str2)) {  // 동등연산자는 주소값 비교
			System.out.println("객체 정보 같음");
			
			
		}else {
			
			System.out.println("객체정보 다름");
		}
		...
		// 2. 리터럴을 사용하여 문자열을 할당하는 방법 (이방법이 주로 사용)
		//  ==> 리터럴을 직접 할당할 경우 상수 풀(pool)이 지정할 문자열이
		// 이미 전조하는지 판별한 후 존재하지 않으면 새로 생성하고,
		// 존재할 경우 기존 문자열의 주소값만 리턴
		// => 즉, 동일한 문자열을 여러 변수에 할당해도 메모리 낭비가 없다!
		
		
		String str3 = "자바";   //그래서 주소값이 같다.
		String str4 = "자바";	////그래서 주소값이 같다.
		
		if(str3 == str4) {  // 동등연산자는 주소값 비교
			System.out.println("주소값 같다");
		...
		if(str3.equals(str4)) {  // 동등연산자는 주소값 비교
			System.out.println("객체 정보 같음");
