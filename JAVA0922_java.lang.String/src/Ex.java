...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 *     멤버변수들에 저장된 값을 의미하지만,
		 *    Object 클래스의 toString(I) 메서드는 객체의 멤버변수 값이 아닌
		 ...
		 *    이 때 출력문에는 toString() 메서드 호출 부분 생략 가능
		...
		// getClass().getName() 의 리턴 값과 '@' 기호와
		// hashCode() 리턴값을 16진수로 변환한 값을 문자열로 결합한 값이다.
	
		System.out.println("--------------------------------");
		
		Person2 p2 = new Person2("홍길동", 20, "909090909090");
		
		// 오버라이딩 된 toString() 메서드 호출
		System.out.println("Person 클래스 객체 p2의 정보 : " + p2.toString());
		// 출력문 내에서 toString() 메서드는 생략 가능
		System.out.println("Person 클래스 객체 p2의 정보 : " + p2);
		//toString 리턴값이 String 타입이므로 String 타입 변수에 저장 가능
		String info = p2.toString();
		//String info2 = p2; // 오류발생! 
